...
import com.htfp.service.oac.dao.mapper.DynamicUavInfoMapper;
...
@Slf4j
@Service
public class DynamicUavInfoDalService {

    @Resource
    DynamicUavInfoMapper dynamicUavInfoMapper;

    public DynamicUavInfoDO queryDynamicUavInfo(Long id){
        return dynamicUavInfoMapper.selectById(id);
    }

    public DynamicUavInfoDO queryDynamicUavInfoByFlyId(Long flyId){
    ...
    public DynamicUavInfoDO queryDynamicUavInfoByReplyFlightPlanId(Long replyFlightPlanId){
        return dynamicUavInfoMapper.selectByReplyFlightPlanId(replyFlightPlanId);
    }

    public List<DynamicUavInfoDO> queryDynamicUavInfoByCpn(String cpn){
        List<DynamicUavInfoDO> dynamicUavInfoDOList = dynamicUavInfoMapper.selectByCpn(cpn);
    ...
    public int insertDynamicUavInfo(DynamicUavInfoDO dynamicUavInfoDO){
        return dynamicUavInfoMapper.insertDynamicUavInfo(dynamicUavInfoDO);
    }

    public int updateDynamicUavInfo(DynamicUavInfoDO dynamicUavInfoDO){
        return dynamicUavInfoMapper.updateByDynamicUavInfo(dynamicUavInfoDO);
    }

    public int deleteDynamicUavInfo(Long id){
        return dynamicUavInfoMapper.deleteById(id);
    }

    public int deleteDynamicUavInfoByReplyFlightPlanId(Long replyFlightPlanId){
        return dynamicUavInfoMapper.deleteByReplyFlightPlanId(replyFlightPlanId);
    }
